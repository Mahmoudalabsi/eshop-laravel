...
# Vercel's PHP 8.3 runtime ships an older SQLite that doesn't support:
#   - pragma_table_xinfo() (added in SQLite 3.26)
#   - 2-arg form of pragma_table_xinfo(table, schema) (added in SQLite 3.37)
# Patch to use pragma_table_info() (available since SQLite 3.16) and a
# hardcoded 0 for the "hidden" column that pragma_table_info doesn't return.
GRAMMAR_FILE="vendor/laravel/framework/src/Illuminate/Database/Schema/Grammars/SQLiteGrammar.php"
if [ -f "$GRAMMAR_FILE" ]; then
  # Replace pragma_table_xinfo with pragma_table_info, and replace hidden column with literal 0
  sed -i \
