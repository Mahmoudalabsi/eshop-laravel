...
class SetupController extends Controller
{
    ...
    public function run()
    {
        ...
        try {
            // Disable FK constraints (SQLite-only; PostgreSQL handles this via migrations)
    ...
    /**
     * Disable FK constraints (SQLite only). On PostgreSQL this is a no-op
     * because migrations handle FK creation correctly and seeding is ordered.
     */
    private function disableFkConstraints(): void
    {
        try {
    ...
    private function enableFkConstraints(): void
    {
        try {
            $driver = DB::connection()->getDriverName();
            if ($driver === 'sqlite') {
